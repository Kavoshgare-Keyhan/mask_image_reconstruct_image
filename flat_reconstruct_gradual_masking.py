...
def main(args):
    ...
    model_vqvae.load_state_dict(torch.load(args.ckpt_vqvae))
    ...
    for perc in mask_percentages:
        ...
        correct_grad_pred = 0
        tot_sample = 0
        for x in range(0,quantizes.shape[0],10):
            print(x)
            ...
            with torch.no_grad():
                q_masked, index_masked, mask = gradual_masking(model_distil, q, index ,perc) 
            ...
            with torch.no_grad():
                ...
                vqvae_masked_out = model_vqvae.decode(torch.from_numpy(quant_b[x]).to(device))
                ...
                print(f'grad_mask_img_label is {grad_mask_img_label}')
                print(f'vqvae_img_label is {vqvae_img_label}')
                print(f'grad_mask_img_label is {grad_mask_img_label.item()}')
                print(f'vqvae_img_label is {vqvae_img_label.item()}')
                print(f'correct_grad_pred is {correct_grad_pred}')
                tot_sample += 1
                print(tot_sample)
                ...
                utils.save_image(
                    ...
                    f"image/recons/gradual/80x80_grad_{vqvae_img_label.item()}_{grad_mask_img_label.item()}_{int(perc*100)}_{str(x).zfill(5)}.png",
                    nrow=3,
                    normalize=True,
                    range=(-1, 1),
                )
            ...
            run["recons/mse_per_image_grad_mask"].log(recon_loss.item())
            reconstruction_errors.append(recon_loss.item())
            class_loss = criterion_class(grad_mask_img_prob, vqvae_img_prob)
            run["recons/cross_entropy_per_image_grad_mask"].log(class_loss.item())
            cross_entropy_class_err.append(class_loss.item())
        run["recons/average_mse_per_precision_grad_mask"].log(np.mean(reconstruction_errors))
        run["recons/average_cross_entropy_error_grad_mask"].log(np.mean(cross_entropy_class_err))
        ...
        run["recons/average_classification_accuracy_gradual"].log(pred_err_grad_mask)
    ...
    plt.title('Reconstruction Error for Gradual Masking vs Mask Percentage')
    plt.grid(True)
    plot_path = 'image/recons/gradual_error_vs_precision.png'
    plt.savefig(plot_path)
    plt.close()
    run['gradual_masking_error_vs_percentage'].upload(plot_path)
